...
/*
 * Samsung Galaxy Book (and other Meteor Lake boards) feed the sensor a 26MHz
 * clock instead of the 19.2MHz the reference design uses. The sensor locks and
 * streams fine on it, so accept it rather than refusing to probe.
 */
#define OV02C10_MCLK_ALT		26000000

/*
 * On the Samsung Galaxy Book the sensor is mounted upside down, but the
 * firmware's SSDB reports a mounting rotation of 0. Userspace (libcamera)
 * owns orientation - it resets HFLIP/VFLIP to match the rotation property -
 * so defaulting the flip controls achieves nothing. Reporting the real
 * mounting rotation is what makes userspace correct the image.
 * Set rotation=0 on boards where the sensor is mounted the usual way.
 */
static bool extend_gain = true;
module_param(extend_gain, bool, 0444);
MODULE_PARM_DESC(extend_gain,
	"Chain digital gain onto the analogue gain control (default: on)");

/*
 * Rolling brightness bands appear when the exposure time is not a whole
 * multiple of the mains half-cycle: the light pulses at twice the mains
 * frequency and each sensor row integrates a different part of the pulse.
 * Measured on this board, the banding drops to ~1% at whole multiples and
 * rises to ~18% at half multiples, so quantise the exposure control to the
 * half-cycle and let the (very wide) gain range do the fine adjustment.
 * Set flicker_hz=0 to expose the full, unquantised exposure range.
 */
static int min_fps = 29;
module_param(min_fps, int, 0444);
MODULE_PARM_DESC(min_fps,
	"Lowest frame rate to trade for exposure time (0 keeps sensor default)");

static int flicker_hz = 60;
module_param(flicker_hz, int, 0444);
MODULE_PARM_DESC(flicker_hz,
	"Mains frequency to avoid banding for: 50, 60, or 0 to disable");

static int rotation = 180;
module_param(rotation, int, 0444);
MODULE_PARM_DESC(rotation, "Sensor mounting rotation in degrees (0 or 180)");
...
/*
 * libcamera's software ISP has no notion of the sensor's digital gain, so on
 * a sensor this insensitive the picture stays badly underexposed once the
 * analogue gain saturates - the hardware can do 16x more, but nothing asks
 * for it. Extend the analogue gain control past the analogue maximum and
 * chain the surplus into digital gain, so a single control drives both and
 * digital gain is only ever used after the analogue range is exhausted
 * (which keeps the signal-to-noise ratio as good as it can be).
 */
#define OV02C10_ANAL_GAIN_EXT_MAX	(OV02C10_ANAL_GAIN_MAX * 16)
...
static int ov02c10_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	if (ctrl->id == V4L2_CID_VBLANK) {
		...
		/*
		 * Re-apply the flicker quantisation: this range is recomputed
		 * from scratch here and would otherwise lose it.
		 */
		if (ov02c10->flicker_lines > 0)
			exposure_max -= exposure_max % ov02c10->flicker_lines;
	...
	case V4L2_CID_ANALOGUE_GAIN: {
		u32 again = ctrl->val;
		u32 dgain = OV02C10_DGTL_GAIN_MIN;

		if (again > OV02C10_ANAL_GAIN_MAX) {
			dgain = DIV_ROUND_CLOSEST(again * OV02C10_DGTL_GAIN_MIN,
						  OV02C10_ANAL_GAIN_MAX);
			dgain = clamp(dgain, (u32)OV02C10_DGTL_GAIN_MIN,
				      (u32)OV02C10_DGTL_GAIN_MAX);
			again = OV02C10_ANAL_GAIN_MAX;
		}
...
static int ov02c10_init_controls(struct ov02c10 *ov02c10)
{
	...
	s64 exposure_max, exposure_min, exposure_step, h_blank, pixel_rate;
	int ret;
	 ...
	 */
	/*
	 * The mode's timing assumes the reference 19.2MHz input clock. On a
	 * board clocked faster the sensor runs proportionally quicker (26MHz
	 * gives ~40fps instead of 30fps), which also caps the exposure time
	 * well below what the mode intends. Scale the frame length by the
	 * clock ratio so the nominal frame rate - and the full exposure
	 * range that comes with it - is restored.
	 */
	vts_def = div_u64((u64)mode->vts_min * ov02c10->mipi_lanes *
			  ov02c10->mclk_rate, OV02C10_MCLK);

	if (flicker_hz == 50 || flicker_hz == 60)
		ov02c10->flicker_lines =
			div_u64(div_u64(pixel_rate * ov02c10->mclk_rate,
					OV02C10_MCLK),
				(u64)mode->hts * 2 * flicker_hz);

	/*
	 * Measured on this sensor at constant output brightness, spending a
	 * frame-rate budget on exposure instead of digital gain is worth a
	 * lot: 8.3ms + 16x digital gain gives SNR 2.3, while 33ms + 4x gives
	 * 4.6. So stretch the frame length to fit as many whole flicker-safe
	 * exposure steps as the frame-rate floor allows.
	 */
	if (ov02c10->flicker_lines > 0 && min_fps > 0) {
		u64 rate = div_u64(pixel_rate * ov02c10->mclk_rate,
				   OV02C10_MCLK);
		u64 lines_at_floor = div_u64(rate, (u64)mode->hts * min_fps);
		u64 steps = div_u64(lines_at_floor - OV02C10_EXPOSURE_MAX_MARGIN,
				    ov02c10->flicker_lines);

		if (steps >= 1) {
			u64 want = steps * ov02c10->flicker_lines +
	...
	/*
	 * Pin the frame length: userspace would otherwise shorten it chasing
	 * a higher frame rate, dragging the exposure ceiling below the
	 * largest flicker-safe step and costing most of the light.
	 */
	vblank_min = vts_def - mode->height;
	...
	v4l2_ctrl_new_std(ctrl_hdlr, &ov02c10_ctrl_ops, V4L2_CID_ANALOGUE_GAIN,
			  OV02C10_ANAL_GAIN_MIN,
			  extend_gain ? OV02C10_ANAL_GAIN_EXT_MAX
				      : OV02C10_ANAL_GAIN_MAX,
	...
	exposure_min = OV02C10_EXPOSURE_MIN;
	exposure_step = OV02C10_EXPOSURE_STEP;

	{
		s64 half = ov02c10->flicker_lines;

		if (half > 0 && half <= exposure_max) {
			exposure_step = half;
			exposure_min = half;
			exposure_max -= exposure_max % half;
			dev_info(ov02c10->dev,
				 "%d Hz flicker avoidance: exposure quantised to %lld lines (max %lld)\n",
				 flicker_hz, half, exposure_max);
		}
	}
	...
	if (!ret && (rotation == 0 || rotation == 180) &&
	    props.rotation != rotation) {
		dev_info(ov02c10->dev,
			 "overriding firmware mounting rotation %u -> %d\n",
			 props.rotation, rotation);
		props.rotation = rotation;
	}
	if (ret)
			      ...
			      struct v4l2_subdev_format *fmt)
{
	...
	*v4l2_subdev_state_get_format(sd_state, fmt->pad) = fmt->format;
	...
	/* Keep the frame length chosen at probe - see ov02c10_init_controls(). */
	vblank_def = ov02c10->vblank->minimum;
	__v4l2_ctrl_modify_range(ov02c10->vblank, vblank_def,
...
static int ov02c10_probe(struct i2c_client *client)
{
	...
	if (freq != OV02C10_MCLK && freq != OV02C10_MCLK_ALT)
		return dev_err_probe(ov02c10->dev, -EINVAL,
				     "external clock %lu is not supported",
				     freq);

	ov02c10->mclk_rate = freq;
	...
	/*
	 * The imaging clock can still report the reference rate this early in
	 * probe - on some boards the provider only settles on the real rate
	 * once the clock has been prepared. Since both rates are accepted, an
	 * early read is silently wrong rather than fatal, and every timing
	 * derived from it (frame length, exposure range) ends up scaled for
	 * the wrong clock. Re-read now that power_on() has enabled it.
	 */
	freq = clk_get_rate(ov02c10->img_clk);
	if (freq != ov02c10->mclk_rate &&
	    (freq == OV02C10_MCLK || freq == OV02C10_MCLK_ALT)) {
		dev_info(ov02c10->dev,
			 "imaging clock settled at %lu Hz (was %lu Hz)\n",
			 freq, ov02c10->mclk_rate);
		ov02c10->mclk_rate = freq;
	}
